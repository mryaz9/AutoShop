...
# Не понятно что делать с action и проверить могут ли быть callback на русском
...
async def categories_keyboard() -> InlineKeyboardBuilder:
    ...
    for category in categories:
        ...
        callback_data = MenuCD(level=CURRENT_LEVEL + 1, category_code=category.category_code,
                               category_name=category.category_name).pack()
...
async def subcategories_keyboard(category_code) -> InlineKeyboardBuilder:
    ...
    for subcategory in subcategories:
        ...
        callback_data = MenuCD(level=CURRENT_LEVEL + 1, category_code=category_code,
                               subcategory_code=subcategory.subcategory_code).pack()
        markup.add(
            InlineKeyboardButton(text=button_text, callback_data=callback_data)
        )
...
async def items_keyboard(category_code, subcategory_code) -> InlineKeyboardBuilder:
    CURRENT_LEVEL: int = 2
    ...
    items = await get_items(category_code, subcategory_code)
    for item in items:
        # Сформируем текст, который будет на кнопке
        ...
        # Сформируем колбек дату, которая будет на кнопке
        callback_data = MenuCD(level=CURRENT_LEVEL + 1, category_code=category_code,
                               subcategory_code=subcategory_code, item_id=item.id).pack()
        markup.add(
...
def item_keyboard(category_code, subcategory_code, item_id) -> InlineKeyboardBuilder:
    CURRENT_LEVEL: int = 3
...
def add_back_and_main(markup: InlineKeyboardBuilder, current_level,
                      category_code="0", subcategory_code="0", item_id="0") -> InlineKeyboardMarkup:
    ...
    markup.add(
        InlineKeyboardButton(
            text=LEXICON["back"],
            callback_data=MenuCD(level=current_level - 1, category_code=category_code,
                                 subcategory_code=subcategory_code, item_id=item_id).pack())
    )
